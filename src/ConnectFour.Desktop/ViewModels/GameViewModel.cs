...
public sealed partial class GameViewModel : ViewModelBase
{
    ...
    {
        foreach (var c in Cells) c.IsLandingPreview = false;
        ...
        var cell = Cells.First(vm => vm.Row == landRow && vm.Column == column);
        var brush = (IBrush)CellStateToBrushConverter.Instance
            .Convert(_game.CurrentPlayer.ToCellState(), typeof(IBrush), null!, CultureInfo.InvariantCulture)!;
